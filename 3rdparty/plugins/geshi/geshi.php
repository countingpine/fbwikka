								...
								foreach ($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark) {
									...
									if ($match) {
										...
										if ($this->lexic_permissions['COMMENTS'][$comment_key]) {
											...
											$test_str = "<span$attributes>" . @htmlspecialchars($this->change_case($test_str, $test_str), ENT_COMPAT, $this->encoding);
										} else {
											$test_str = @htmlspecialchars($test_str, ENT_COMPAT, $this->encoding);
										}
						...
						if (false !== stristr($stuff_to_parse_pregquote, $keyword )) {
							...
							if ($this->language_data['CASE_SENSITIVE'][$k]) {
								$stuff_to_parse = preg_replace(
                                    ...
                                    "'\\1' . $func2('\\2', '$k', 'BEGIN') . '<|$styles>' . $func('\\2', '$keyword') . '|>' . $func2('\\2', '$k', 'END')",
                                    $stuff_to_parse
                                );
							} else {
